...
Nightwatch.prototype.start = function() {
  if (!this.sessionId && this.options.start_session) {
    this
      ...
      .startSession(this.options.session_retries);
    return this;
  }
...
Nightwatch.prototype.startSession = function (attempts) {
  const attemptsLeft = (attempts || 1) -1;
  if (this.terminated) {
    return this;
  }
  var self = this;
  ...
  request.on('success', function(data, response, isRedirect) {
    if (data && data.sessionId) {
      ...
      if (attemptsLeft > 0) {
        console.log(Logger.info(`Attempt to create session failed ${attemptsLeft} attempts left`));
        self.startSession(attemptsLeft);
      } else {
  ...
  .on('error', function(data, err) {
    if (attemptsLeft > 0) {
      console.log(Logger.colors.yellow(`\nAttempt to create session failed ${attemptsLeft} attempts left`));
